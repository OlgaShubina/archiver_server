...
def clear_storage():
	...
	for i in array:
		...
		if app.AsyncResult(i.decode("utf-8")).ready() == True:
			...
			archive.delete_raw((datetime.datetime.strptime(r[b't1'].decode("utf-8"),'%Y-%m-%d %H:%M:%S')).timestamp(),(datetime.datetime.strptime(r[b't2'].decode("utf-8"),'%Y-%m-%d %H:%M:%S')).timestamp())
